...
def salvar_dados(nome, idade, telefone, cidade, sexo, part_ultima_corrida):
    try:
        with open("dados.csv", "a+", encoding="utf-8") as file:
            file.write(f"{nome},{idade},{telefone},{cidade},{sexo},{part_ultima_corrida}\n")
        return True
...
def validar_dados(nome, idade, telefone, cidade, sexo, part_ultima_corrida):
    ...
    if not str(telefone).isdigit() or len(telefone) < 10:
...
nome = nome.title()
idade = st.text_input("Digite sua idade:", max_chars=2)
...
cidade = st.text_input("Qual sua cidade, comunidade ou povoado?",
                       max_chars=35)
cidade = cidade.title()
sexo = st.pills("Selecione seu sexo",
         ["Masculino", "Feminino"])

part_ultima_corrida = st.pills("Você participou da ultima corrida e café?",
                         ["Sim", "Não"])
...
if st.button("Confirmar"):
    if validar_dados(nome, idade, telefone, cidade, sexo, part_ultima_corrida):
        if salvar_dados(nome, idade, telefone_formatado, cidade, sexo, part_ultima_corrida):
            ...
            with modal.container():
                st.link_button("Entre no grupo do WhatsApp!",
                               url="https://chat.whatsapp.com/KJWgrjXKjEu4YoDyD4VvZe",
